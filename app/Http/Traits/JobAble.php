...
trait JobAble
{
    ...
    {
        if (config('zakirsoft.default_job_provider') == 'indeed' && config('zakirsoft.indeed_id')) {
    ...
    {
        if (config('zakirsoft.default_job_provider') == 'careerjet' && config('zakirsoft.careerjet_id')) {
            $keyword = $request->keyword ?? '';
            // $id = JobCategory::where('slug', $slug)->first();
            // $category_id = $id->id;
            // $category = $category_id ? JobCategory::whereId($category_id)->first() : '';
