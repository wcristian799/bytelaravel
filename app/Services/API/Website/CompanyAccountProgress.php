    ...
    public function submitAccountProgressData($request)
    {
        ...
        try {
            switch ($request->step) {
                case 'company':
                    // return $request->all();
                    $image_validation = $request->iamge && $company->logo ? 'sometimes|image|mimes:jpeg,png,jpg|max:2048' : 'required|image|mimes:jpeg,png,jpg|max:2048';
                    $banner_validation = $request->banner && $company->banner ? 'sometimes|image|mimes:jpeg,png,jpg|max:5120' : 'required|image|mimes:jpeg,png,jpg|max:5120';
