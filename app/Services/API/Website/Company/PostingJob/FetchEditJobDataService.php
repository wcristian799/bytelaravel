...
class FetchEditJobDataService
{
    ...
    public function execute($job)
    {
        ...
        $data['job'] = $job;
        $data['jobCategories'] = JobCategory::all()->map(fn ($data) => ['id' => $data->id, 'name' => $data->name])->sortBy('name');
        $data['roles'] = JobRole::all()->map(fn ($data) => ['id' => $data->id, 'name' => $data->name])->sortBy('name');
        $data['benefits'] = Benefit::all()->map(fn ($data) => ['id' => $data->id, 'name' => $data->name])->sortBy('name');
        $data['tags'] = Tag::all()->map(fn ($data) => ['id' => $data->id, 'name' => $data->name])->sortBy('name');
