...
class JobsImport implements ToModel, WithBatchInserts, WithChunkReading, WithStartRow
{
    ...
    {
        if (isset($data) && ! empty($data)) {
            $experience = Experience::where('name', 'LIKE', "%$data%")->orWhere('slug', 'LIKE', "%$data%")->orWhere('id', 'LIKE', "%$data%")->first();
    ...
    {
        if (isset($data) && ! empty($data)) {
            $education = Education::where('name', 'LIKE', "%$data%")->orWhere('slug', 'LIKE', "%$data%")->orWhere('id', 'LIKE', "%$data%")->first();
    ...
    {
        if (isset($data) && ! empty($data)) {
            $salary_type = SalaryType::where('name', 'LIKE', "%$data%")->orWhere('slug', 'LIKE', "%$data%")->orWhere('id', 'LIKE', "%$data%")->first();
