                                            ...
                                            @if ($job->status == 'active' || $job->status == 'expired')
                                                <div class="custom-control custom-radio custom-control-inline">
                                                    <form action="{{ route('admin.job.status.change', $job->id) }}" method="POST" id="job_status_unpublish_form_{{ $job->id }}">
                                                        @csrf
                                                        @method('PUT')
                                                        <input disabled onclick="$('#job_status_unpublish_form_{{ $job->id }}').submit()" type="radio" id="status_input_unpublish_{{ $job->id }}" name="status"
                                                            class="plan_type_selection custom-control-input" value="expired"
                                                            {{ $job->status == 'expired' ? 'checked' : '' }}>
                                                            <label class="custom-control-label {{$job->status == 'expired'? 'expired_radio' : ''}}" data-toggle="tooltip" title="{{__('expired_status_depend_on_deadline')}}" for="status_input_unpublish_{{ $job->id }}">{{__('expired')}}</label>
                                                    </form>
                                                </div>
                                            @endif
                                        </div>
